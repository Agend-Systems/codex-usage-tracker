...
struct MenuBarLabel: View {
  ...
  var body: some View {
    if let usage {
        ...
        HStack(spacing: 4) {
          ZStack {
            ...
            Circle()
              ...
              .stroke(
                TrackerDesign.usageColor(usage), style: StrokeStyle(lineWidth: 2.4, lineCap: .round)
              )
      ...
      case .compact:
        StatusDot(color: TrackerDesign.usageColor(usage))
      }
    } else {
      switch preferences.iconStyle {
      case .compact:
        StatusDot(color: store.activeState == .connecting ? TrackerDesign.amber : TrackerDesign.red)
      case .ring:
        HStack(spacing: 4) {
